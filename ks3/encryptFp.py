...
class EncryptFp (object):
    ...
    #         #For multi, the first part's first part will add a prefix of iv.
    #             if not self.calc_iv:
    #                 encrypt_data = crypt_handler.encrypt_without_padding(data,self.first_iv)
    #                 encrypt_data = self.first_iv+encrypt_data
    #             else:
    #                 encrypt_data = crypt_handler.encrypt(data,self.calc_iv)
    ...
    #                 encrypt_data = crypt_handler.encrypt_without_padding(data,self.first_iv)
    #             else:
    #                 encrypt_data = crypt_handler.encrypt_without_padding(data,self.calc_iv)
    #         else:
    #         #And the last part's parts use 'encrypt' instead of 'encrypt_without_padding' because the last part's last part need paddling.
    #             if not self.calc_iv:
    #                 encrypt_data = crypt_handler.encrypt_without_padding(data,self.first_iv)
    #             else:
    #                 encrypt_data = crypt_handler.encrypt(data,self.calc_iv)
